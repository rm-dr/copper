...
mod tests {
	...
	pub enum FlacTestCase {
		Success {
			/// A cosmetic name for this test case
	...
	impl FlacTestCase {
		pub fn get_name(&self) -> &str {
			match self {
				Self::Success { test_name, .. } => test_name,
			}
		}

		pub fn get_path(&self) -> &str {
			match self {
				Self::Success { file_path, .. } => file_path,
			}
		}

		pub fn get_in_hash(&self) -> &str {
			match self {
				Self::Success { in_hash, .. } => in_hash,
			}
		}

		pub fn get_stripped_hash(&self) -> &str {
			match self {
				Self::Success { stripped_hash, .. } => stripped_hash,
			}
		}

		pub fn get_audio_hash(&self) -> &str {
			match self {
				Self::Success { audio_hash, .. } => audio_hash,
			}
		}

		pub fn get_blocks(&self) -> &[FlacBlockOutput] {
			match self {
				Self::Success { blocks, .. } => blocks,
			}
		}
	}
